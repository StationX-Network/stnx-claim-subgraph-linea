...
import { handleAirdropClaimed } from "../src/claim-emitter"
import { createAirdropClaimedEvent } from "./claim-emitter-utils"
...
describe("Describe entity assertions", () => {
  beforeAll(() => {
    let claimContract = Address.fromString(
      "0x0000000000000000000000000000000000000001"
    )
    let user = Address.fromString("0x0000000000000000000000000000000000000001")
    let token = Address.fromString("0x0000000000000000000000000000000000000001")
    let claimableAmount = BigInt.fromI32(234)
    let airdropAmount = BigInt.fromI32(234)
